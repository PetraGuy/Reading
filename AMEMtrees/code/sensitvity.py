...
groundflora.loc[groundflora.CoverYr1 <=1,'CoverYr1']=groundflora['CoverYr1']*100
groundflora.loc[groundflora.CoverYr2 <=1,'CoverYr2']=groundflora['CoverYr2']*100 

#but not sure, 1 might be 1 and 0.5 might be 50 or 0.5.
...
#combine with herbs so have cover and richness for each species
herb_cover_richness = pd.merge(herbflora,herbrichness, on = ['Site','Plot','Yr'], how = 'left')
#save for later
herbrichness.to_csv('../data/herbrichness.csv', index = False)
herb_cover_richness.to_csv('../data/herb_cover_richness.csv',index = False)
###################################################################

#merge the sbrubcover with a richness to get a file to use for invasives analysis
shrub_cover_richness = pd.merge(shrubcoverinvasive,herbrichness, on = ['Site','Plot','Yr'],how = 'left')

#the nas are zero richness plots, have checked this
...
filtered_df = shrub_cover_richness[shrub_cover_richness['alpha'].isnull()]
