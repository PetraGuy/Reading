...
#am dropping inv but keeping pH
...
#####################################################
# #remove very invaded plots if required
# emplotsuninvaded = emplots%>%filter(percentinvcover<0.2) #change this for invaded or not
# 
# #split into years again, reduce number of vars as fewer rows, take out pH etc
# emplotsyr1 = emplotsuninvaded%>%filter(Yr==1)
# emplotsyr1 = emplotsyr1[-c(2)]
# emplotsyr2 = emplotsuninvaded%>%filter(Yr==2)
# emplotsyr2 = emplotsyr2[-c(2)]
# 
# 
# #make the sites into a list and loop thorugh list
# emplotsyr1list = emplotsyr1%>%group_by(Site)%>%group_split()
# emplotsyr2list = emplotsyr2%>%group_by(Site)%>%group_split()
...
getaves = function(asite){
  ...
  colnames(testdf) = c("alpha","propAMtree","propAMcover","pH2")
...
getsamples = function(siteslist){
  #browser()
  df = data.frame(matrix(ncol = 4,nrow = length(siteslist)))
  colnames(df)= c("alpha","propAMtree","propAMcover","pH2")
  for (n in 1:length(siteslist)){
    ...
    if (nrow(site)>5){
      ...
      thisrow = vector(mode = 'numeric', length = 4)
      df[n,] = thisrow
    }
  }
...
getfits = function(data){
  ...
  modeldata = as.data.frame(cbind(data$alpha,scaled$pH2,am))
