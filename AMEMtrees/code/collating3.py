...
dbhcounts['percentam']=dbhcounts['amdbhcounts']/dbhcounts['totaldbh'] #amount am trees
...
invshrubcover = invshrub.groupby(['Site','Plot','Yr'])['cover'].sum().to_frame().reset_index()

invherb = herbflora[herbflora['flag']==1]
invherbcover = invherb.groupby(['Site','Plot','Yr'])['cover'].sum().to_frame().reset_index()
...
shrubcover = pd.merge(shrubcover, invshrubcover, on = ['Site','Plot','Yr'],how ='left')
shrubcover = shrubcover.rename(columns={'cover':'invshrubcover'})

shrubcover = pd.merge(shrubcover, invherbcover, on = ['Site','Plot','Yr'],how ='left')
shrubcover = shrubcover.rename(columns={'cover':'invherbcover'})

#for invasives analysis, make another df with species info
#invcover2 = invshrub.groupby(['Site','Plot','Yr','Amalgams'])['cover'].sum().to_frame().reset_index()
#invcover2.to_csv('../data/invasivecover.csv',index = False)
####################################################################
...
alldata['percentinvshrubcover']= alldata['invshrubcover']/alldata['totashrublcover']
alldata['invasion'] = alldata['invshrubcover']+alldata['invherbcover']
